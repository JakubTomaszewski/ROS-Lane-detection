...
def hist_calculation(image):
    ...
    if np.sum(hist) == 0:
        raise IncorrectImage('Too dark image, check if the camera is not obstructed')
...
def calc_line_fits(img, ym_per_pix=30/720, xm_per_pix=3.7/550):
    ...
    try:
        ...
        raise
    except IncorrectImage:
        raise
    except ValueError as e:
        raise IncorrectImage(e)
...
def get_center_dist(leftLine, rightLine, height, width, xm_per_pix=3.7/550, margin_y=20):
    ...
    try:
        ...
        left_pos = leftLine.best_fit_px[0]*(y**2) + leftLine.best_fit_px[1]*y + leftLine.best_fit_px[2]
        right_pos = rightLine.best_fit_px[0]*(y**2) + rightLine.best_fit_px[1]*y + rightLine.best_fit_px[2]
        lane_middle = (right_pos - left_pos) // 2 + left_pos
...
def create_final_image(img, binary_warped, leftLine, rightLine, warper):
    ...
    new_warp = warper.inverse_warp_matrix(color_warp)
